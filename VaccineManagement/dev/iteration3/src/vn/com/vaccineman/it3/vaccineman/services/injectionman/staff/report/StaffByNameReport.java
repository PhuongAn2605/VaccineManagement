package vn.com.vaccineman.it3.vaccineman.services.injectionman.staff.report;
...
public class StaffByNameReport {
	 @DAttr(name = "id", id = true, auto = true, type = Type.Integer, length = 5, optional = false, mutable = false)
	  ...
	  @DAttr(name="staff",type=Type.Collection,optional=false, mutable=false,
	      serialisable=false,filter=@Select(clazz=Staff.class, 
	      attributes={Staff.A_id, Staff.A_name, Staff.A_dob, Staff.A_address, 
	          Staff.A_phoneNumber, Staff.A_rptStaffByName})
	      ,derivedFrom={"name"}
	      )
	  @DAssoc(ascName="staff-by-name-report-has-staff",role="report",
	      ascType=AssocType.One2Many,endType=AssocEndType.One,
	    associate=@Associate(type=Staff.class,cardMin=0,cardMax=MetaConstants.CARD_MORE
	    ))
	  @Output
	  private Collection<Staff> staff;

	  /**output: number of staff found (if any), derived from {@link #staff} */
	  @DAttr(name = "numStaff", type = Type.Integer, length = 20, auto=true, mutable=false)
	  @Output
	   ...
	   *  all {@link Staff} whose names match <tt>name</tt>.
	   *  initialise {@link #staff} with the result if any.
	   *  
	  ...
	  public StaffByNameReport(@AttrRef("name") String name) throws NotPossibleException, DataSourceException {
	    this.id=++idCounter;
...
//	    boolean doReportQuery = (name != null && !name.equals(this.name));
	  ...
	  public void doReportQuery() throws NotPossibleException, DataSourceException {
	    ...
	    // create a query to look up Staff from the data source
	    // and then populate the output attribute (staff) with the result
	    DSMBasic dsm = qrm.getDsm();
	    
	    //TODO: to conserve memory cache the query and only change the query parameter value(s)
	    Query q = QueryToolKit.createSearchQuery(dsm, Staff.class, 
	        new String[] {Staff.A_name}, 
	        new Op[] {Op.MATCH}, 
	        new Object[] {"%"+searchName+"%"});
	    
	    Map<Oid, Staff> result = qrm.getDom().retrieveObjects(Staff.class, q);
	  ...
	  /**
	   * A link-adder method for {@link #staff}, required for the object form to function.
	   * However, this method is empty because staff have already be recorded in the attribute {@link #staff}.
	   */
	  @DOpt(type=DOpt.Type.LinkAdder)
	  public boolean addStaff(Collection<Staff> staff) {
	    // do nothing
	    return false;
	  }
	  
	  /**
	   * @effects return staff
	   */
	  ...
	  public boolean equals(Object obj) {
	    ...
	    if (getClass() != obj.getClass())
	      return false;
	    StaffByNameReport other = (StaffByNameReport) obj;
	    if (id != other.id)
	      return false;
	    return true;
	  }
	  ...
	  public String toString() {
	    return "StaffByNameReport (" + id + ", " + name + ")";
	  }
